...
public class EmpPersonalDetailsFinderImpl extends
		BasePersistenceImpl<EmpPersonalDetails> implements
		EmpPersonalDetailsFinder {
	...
	public List<EmpPersonalDetails> getEmployeesByShiftId(long shiftId) {
		...
		System.out
				.println("inside getEmployeesByShiftId shiftId == " + shiftId);

		Session session = null;
		try {
			session = openSession();

			String sql = "SELECT * FROM emp_personal_details ep JOIN emp_job ej WHERE ep.employeeId=ej.employeeId AND ej.shiftId="
					+ shiftId;
			...
			List<EmpPersonalDetails> empPersonalDetailslist = (List<EmpPersonalDetails>) QueryUtil
					.list(q, getDialect(), -1, -1);
